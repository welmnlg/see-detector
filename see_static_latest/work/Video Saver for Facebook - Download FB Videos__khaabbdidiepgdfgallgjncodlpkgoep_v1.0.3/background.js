var background = (function() {
  ...
  const OFFSCREEN_URL = "unlisted-offscreen.html";
  ...
  async function ensureOffscreenDocument() {
    ...
    creatingOffscreen = chrome.offscreen.createDocument({
      ...
      justification: "FFmpeg video/audio merge requires Web Workers"
    }).finally(() => {
  ...
  const definition = defineBackground(() => {
    chrome.runtime.onMessage.addListener((request, _sender, sendResponse) => {
      if (request.action === "openUrl") {
        (async () => {
          try {
            const { url, tabPayload, tabAction } = request.payload;
            const newTab = await chrome.tabs.create({ url, active: true });
            if (tabPayload || tabAction) {
            ...
            console.error("Failed to handle openUrl action", error);
      ...
      if (request.action === "signInWithGoogle") {
        (async () => {
          try {
            const { signInWithGoogle: signInWithGoogle2 } = await Promise.resolve().then(() => signInWithGoogle$1);
            await signInWithGoogle2(request.payload?.nextUrl);
      ...
      if (request.action === "mergeAndDownload") {
        (async () => {
          try {
            ...
            console.error("Failed to merge and download:", error);
            sendResponse({
    ...
    chrome.runtime.onInstalled.addListener((details) => {
      if (details.reason === chrome.runtime.OnInstalledReason.INSTALL) {
        chrome.runtime.setUninstallURL("https://forms.gle/5Y2msLvYxW9terAC6");
      }
    });
  });
...
Option 2: Install and provide the "ws" package:
  ...
  const client = new RealtimeClient(url, {
    ...
    _initializeOptions(options) {
      ...
      const result2 = {};
      result2.timeout = (_c = options === null || options === void 0 ? void 0 : options.timeout) !== null && _c !== void 0 ? _c : DEFAULT_TIMEOUT$1;
      ...
      result2.transport = (_e = options === null || options === void 0 ? void 0 : options.transport) !== null && _e !== void 0 ? _e : WebSocketFactory.getWebSocketConstructor();
...
${suffix}`;
  ...
  const storage$1 = new g({
    area: "local"
  });
  const supabase = createClient(
    "https://zjtlssppxzesxvhwokje.supabase.co",
    "eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6InpqdGxzc3BweHplc3h2aHdva2plIiwicm9sZSI6ImFub24iLCJpYXQiOjE3MTQxNDY2MTgsImV4cCI6MjAyOTcyMjYxOH0.JcHngMZWxfw6Wel2DWn30RzOoMCEDlP4lQq1osOB400",
    {
      auth: {
        storage: storage$1,
        autoRefreshToken: true,
  ...
  supabase.auth.onAuthStateChange(async (event, session) => {
    if (event === "SIGNED_OUT") {
      storage$1.remove("userData");
      storage$1.remove("isPaid");
    }
  });
  const storage = new g({ area: "local" });
  async function signInWithGoogle(nextUrl) {
    const url = new URL("https://ext.lazytechstudio.com/success");
    if (nextUrl) {
      url.searchParams.set("nextUrl", nextUrl);
    }
    ...
    chrome.tabs.onUpdated.addListener(
      async function listener(tabId, changeInfo, currentTab) {
        if (tabId !== tab.id || changeInfo.status !== "complete" || !currentTab.url) {
          return;
        }
        ...
        const authStateChangeListener = supabase.auth.onAuthStateChange(
          async (event) => {
            if (event === "SIGNED_IN") {
              await storage.setItem("login_time", (/* @__PURE__ */ new Date()).toUTCString());
        ...
        const nextUrlFromQuery = searchParams.get("nextUrl");
        if (nextUrlFromQuery) {
          const { data: userData } = await supabase.auth.getUser();
          const email = userData.user?.email ?? "";
          const targetUrl = nextUrlFromQuery.replace("{email}", email);
  ...
  const signInWithGoogle$1 = /* @__PURE__ */ Object.freeze(/* @__PURE__ */ Object.defineProperty({
    __proto__: null,
    signInWithGoogle
