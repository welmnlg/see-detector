var post = (function() {
  ...
  async function loadCss() {
    const url = browser.runtime.getURL(`/content-scripts/${"post"}.css`);
    try {
  ...
  function extractVideoIdFromUrl(url) {
    const targetUrl = typeof window !== "undefined" && window.location ? window.location.href : "";
    if (!targetUrl) {
      return null;
    }
    let parsedUrl;
...
`, D = 16, Fe = ({ reverseOrder: e2, position: t2 = "top-center", toastOptions: o2, gutter: s2, children: a2, toasterId: i2, containerStyle: r, containerClassName: l2 }) => {
  ...
  function PostVideoDownloader({
    ...
    const handleDownload = reactExports.useCallback(async () => {
      ...
      if (!videoId) {
        console.warn("[PostDL] videoId not found");
      ...
      try {
        ...
        console.error("[PostDL] download failed:", e2);
  ...
  const definition = defineContentScript({
    ...
    async main(ctx) {
      ...
      const tryMount = async () => {
        ...
        const isPostPath = pathname === "/permalink.php" || pathname.includes("/posts/") || pathname.includes("/videos/");
        if (!isPostPath) return;
        const { selector, insertPosition } = await getSelectorConfig("post");
        if (!selector) return;
        ...
        ui = await createShadowRootUi(ctx, {
          ...
          onMount(container) {
            ...
            root2.render(
              React.createElement(PostVideoDownloader, {
  ...
  function getUniqueEventName(eventName) {
    return `${browser?.runtime?.id}:${"post"}:${eventName}`;
  }
  ...
  const result = (async () => {
    try {
      ...
      return await main(new ContentScriptContext("post", options));
    } catch (err) {
      logger.error(`The content script "${"post"}" crashed on startup!`, err);
      throw err;
    }
  })();
  return result;
