var reels = (function() {
  ...
  async function loadCss() {
    const url = browser.runtime.getURL(`/content-scripts/${"reels"}.css`);
    try {
...
`, D = 16, Fe = ({ reverseOrder: e2, position: t2 = "top-center", toastOptions: o2, gutter: s2, children: a2, toasterId: i2, containerStyle: r, containerClassName: l2 }) => {
  ...
  function ReelsVideoDownloader({
    ...
    const handleDownload = reactExports.useCallback(async () => {
      ...
      const videoContainer = anchorElement.closest(
        "div.x6s0dn4.x78zum5.x1n2onr6"
      );
      if (!videoContainer) {
        console.warn("[ReelsDL] videoContainer not found");
        showDownloadError(getStageMessage("NO_VIDEO_ID"));
        return;
      }
      const videoId = videoContainer.querySelector("[data-video-id]")?.getAttribute("data-video-id");
      if (!videoId) {
        console.warn("[ReelsDL] videoId not found");
        showDownloadError(getStageMessage("NO_VIDEO_ID"));
        return;
      }
      ...
      try {
        ...
        console.error("[ReelsDL] download failed:", e2);
        showDownloadError(getStageMessage(e2.stage));
      } finally {
        setDownloading(false);
      }
    }, [anchorElement, downloading]);
    return /* @__PURE__ */ jsxRuntimeExports.jsxs(jsxRuntimeExports.Fragment, { children: [
      /* @__PURE__ */ jsxRuntimeExports.jsx(
        "button",
        {
          "aria-label": "ReelsDL downloader",
          className: classnames(
            "mx-auto relative z-[2147483647]",
  ...
  const definition = defineContentScript({
    ...
    async main(ctx) {
      ...
      const checkAndMount = async () => {
        ...
        for (let i2 = mountedUIs.length - 1; i2 >= 0; i2--) {
          if (!document.contains(mountedUIs[i2].anchor)) {
            mountedUIs[i2].ui.remove();
            mountedUIs.splice(i2, 1);
          }
        }
        const { selector, insertPosition } = await getSelectorConfig("reels");
        if (!selector) return;
        const elements = document.querySelectorAll(selector);
        if (elements.length === 0) return;
        for (const element of elements) {
          if (mountedUIs.some((m2) => m2.anchor === element)) continue;
          const ui = await createShadowRootUi(ctx, {
            ...
            onMount(container) {
              ...
              root2.render(
                React.createElement(ReelsVideoDownloader, {
          ...
          ui.mount();
          mountedUIs.push({ anchor: element, ui });
        }
      };
      await checkAndMount();
      const interval = setInterval(checkAndMount, 1e3);
      ctx.onInvalidated(() => {
        clearInterval(interval);
        mountedUIs.forEach((m2) => m2.ui.remove());
      });
    }
  });
  ...
  function getUniqueEventName(eventName) {
    return `${browser?.runtime?.id}:${"reels"}:${eventName}`;
  }
  ...
  const result = (async () => {
    try {
      ...
      return await main(new ContentScriptContext("reels", options));
    } catch (err) {
      logger.error(`The content script "${"reels"}" crashed on startup!`, err);
      throw err;
    }
  })();
  return result;
