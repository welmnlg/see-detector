var watch = (function() {
  ...
  async function loadCss() {
    const url = browser.runtime.getURL(`/content-scripts/${"watch"}.css`);
    try {
...
`, D = 16, Fe = ({ reverseOrder: e2, position: t2 = "top-center", toastOptions: o2, gutter: s2, children: a2, toasterId: i2, containerStyle: r, containerClassName: l2 }) => {
  ...
  function WatchVideoDownloader({
    ...
    const handleDownload = reactExports.useCallback(async () => {
      ...
      let videoId = null;
      if (parentSelector && videoUrlSelector) {
        const parent = anchorElement.closest(parentSelector);
        const linkEl = parent?.querySelector(videoUrlSelector);
        if (linkEl?.href) {
          videoId = extractVideoIdFromUrl(linkEl.href);
        }
      }
      if (!videoId) {
      ...
      if (!videoId) {
        console.warn("[WatchDL] videoId not found");
      ...
      try {
        ...
        console.error("[WatchDL] download failed:", e2);
        showDownloadError(getStageMessage(e2.stage));
      } finally {
        setDownloading(false);
      }
    }, [anchorElement, parentSelector, videoUrlSelector, downloading]);
    return /* @__PURE__ */ jsxRuntimeExports.jsxs(jsxRuntimeExports.Fragment, { children: [
      !hasBackdrop && hovered && /* @__PURE__ */ jsxRuntimeExports.jsx(
        "button",
        {
          "aria-label": "WatchDL downloader",
  ...
  const definition = defineContentScript({
    ...
    async main(ctx) {
      ...
      const tryMount = async () => {
        ...
        const { selector, insertPosition, videoUrlSelector, parentSelector } = await getSelectorConfig("watch");
        if (!selector) return;
        const elements = document.querySelectorAll(selector);
        for (const element of elements) {
          if (mountedMap.has(element)) continue;
          const ui = await createShadowRootUi(ctx, {
            name: "watch-downloader",
            position: "inline",
            anchor: element,
            ...
            onMount(container) {
              ...
              root2.render(
                React.createElement(WatchVideoDownloader, {
  ...
  function getUniqueEventName(eventName) {
    return `${browser?.runtime?.id}:${"watch"}:${eventName}`;
  }
  ...
  const result = (async () => {
    try {
      ...
      return await main(new ContentScriptContext("watch", options));
    } catch (err) {
      logger.error(`The content script "${"watch"}" crashed on startup!`, err);
      throw err;
    }
  })();
  return result;
