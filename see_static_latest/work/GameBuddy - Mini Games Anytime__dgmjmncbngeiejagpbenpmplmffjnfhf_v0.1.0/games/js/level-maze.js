/*
 * Level Maze Game - Version 2
 * Rewritten using ES6 classes and different code structure
 * Similar to blocker-maze but without obstacles
 */
...
const LEVEL_STORAGE_KEY = 'maze_game_level'; // Use chrome.storage.local
const WALL_SPACE = 2;
const BOX_SPACE = 2;
const SDPAD = 12;
...
class LevelMazeGame {
    constructor() {
        ...
        // Player position (matching original: currentLine is x, currentCol is y)
        this.currentLine = 0;
        this.currentCol = 0;
    ...
    startGame(skipAdaptiveUpdate = false) {
        ...
        const blockSizeMax = Math.min(
            this.screenWidth - this.sidebarWidth - SDPAD * 2 - 20,
            this.screenHeight - this.topbarHeight - SDPAD * 2 - 20
        );
        this.cellSize = Math.floor(blockSizeMax / this.gridSize);
        this.boxSize = this.cellSize - WALL_SPACE * 2;
        this.canvasSize = this.gridSize * this.cellSize + SDPAD * 2;
    ...
    updateUIDimensions() {
        const topbar = document.getElementById('topbar');
        if (topbar) {
            topbar.style.paddingLeft = `${SDPAD}px`;
            topbar.style.paddingRight = `${SDPAD}px`;
    ...
    generateMaze() {
        ...
        const drawline = (a, b) => {
            ...
            if (y1 - y2 === 0) {
                ...
                this.ctx.clearRect(
                    SDPAD + (x3 + 1) * this.cellSize - 1,
                    SDPAD + y2 * this.cellSize + 1,
                    WALL_SPACE,
                ...
                this.ctx.clearRect(
                    x1 * this.cellSize + SDPAD + 1,
                    SDPAD + ((y1 + y2) / 2 + 0.5) * this.cellSize - 1,
                    this.cellSize - WALL_SPACE,
                    WALL_SPACE
                );
            }
    ...
    drawGrid() {
        ...
        for (let i = 0; i < this.gridSize + 1; i++) {
            ...
            this.ctx.moveTo(SDPAD + i * this.cellSize, SDPAD);
            this.ctx.lineTo(SDPAD + i * this.cellSize, SDPAD + this.cellSize * this.gridSize);
            this.ctx.stroke();
            
            // Horizontal lines (matching original: moveTo then lineTo then stroke)
            this.ctx.moveTo(SDPAD, SDPAD + i * this.cellSize);
            this.ctx.lineTo(SDPAD + this.cellSize * this.gridSize, SDPAD + i * this.cellSize);
            this.ctx.stroke();
        }
    }
    ...
    drawStartEnd() {
        ...
        // Draw start position (red) - currentLine is x, currentCol is y
        this.ctx.fillStyle = 'red';
        this.ctx.fillRect(
            this.currentLine * this.cellSize + BOX_SPACE + SDPAD,
            this.currentCol * this.cellSize + BOX_SPACE + SDPAD,
            this.boxSize,
            this.boxSize
        );
        
        // Draw end position (green) - currentLine is x, currentCol is y
        this.ctx.fillStyle = 'green';
        this.ctx.fillRect(
            (this.gridSize - 1) * this.cellSize + BOX_SPACE + SDPAD,
            (this.gridSize - 1) * this.cellSize + BOX_SPACE + SDPAD,
            this.boxSize,
            this.boxSize
        );
    ...
    handleKeyPress(e) {
        ...
        // Allow player movement even when paused (only timer is paused)
        const tempLine = this.currentLine;
        const tempCol = this.currentCol;
        let moved = false;
        ...
        switch (e.keyCode) {
            ...
            case 65: // A
                this.currentLine--;
                if (this.currentLine < 0 || this.currentLine >= this.gridSize || 
                    !this.checkWay(tempLine, tempCol, this.currentLine, this.currentCol)) {
                    this.currentLine++;
                } else {
                    moved = true;
                }
            ...
            case 87: // W
                this.currentCol--;
                if (this.currentCol < 0 || this.currentCol >= this.gridSize || 
                    !this.checkWay(tempLine, tempCol, this.currentLine, this.currentCol)) {
                    this.currentCol++;
                } else {
                    moved = true;
        ...
        if (moved) {
            ...
            if (this.shortestPath && this.shortestPath.length > 0) {
                this.clearPathCell(tempLine, tempCol, this.currentLine, this.currentCol);
            }
            
            // Update player position
            this.ctx.clearRect(
                tempLine * this.cellSize + BOX_SPACE + SDPAD,
                tempCol * this.cellSize + BOX_SPACE + SDPAD,
                this.boxSize,
                this.boxSize
            );
            this.ctx.fillStyle = 'red';
    ...
    checkWay(tempLine, tempCol, currentLine, currentCol) {
        const oldSeq = tempCol * this.gridSize + tempLine;
        const newSeq = currentCol * this.gridSize + currentLine;
        return this.way[oldSeq][newSeq] === 1 || this.way[newSeq][oldSeq] === 1;
    }
    
    // Check if player reached end
    checkSuccess() {
        if (this.currentLine === this.gridSize - 1 && this.currentCol === this.gridSize - 1) {
            this.win();
        }
    }
    ...
    showSolution() {
        ...
        const startPosition = this.currentCol * this.gridSize + this.currentLine; // matching original: currentCol * BKNUM + currentLine
    ...
    drawPath(path, color) {
        ...
        for (let i = 0; i < path.length - 1; i++) {
            ...
            const startX = x1 * this.cellSize + BOX_SPACE + SDPAD + this.boxSize / 2;
            const startY = y1 * this.cellSize + BOX_SPACE + SDPAD + this.boxSize / 2;
            const endX = x2 * this.cellSize + BOX_SPACE + SDPAD + this.boxSize / 2;
            const endY = y2 * this.cellSize + BOX_SPACE + SDPAD + this.boxSize / 2;
    ...
    clearPath(path) {
        ...
        for (let i = 0; i < path.length - 1; i++) {
            ...
            // Clear cell area
            this.ctx.clearRect(
                startX - this.boxSize / 2,
                startY - this.boxSize / 2,
                this.boxSize,
                this.boxSize
            );
            
            // Redraw player if at start position - currentLine is x, currentCol is y
            if (this.currentCol === y1 && this.currentLine === x1) {
                this.ctx.fillStyle = 'red';
                this.ctx.fillRect(
                    this.currentLine * this.cellSize + BOX_SPACE + SDPAD,
                    this.currentCol * this.cellSize + BOX_SPACE + SDPAD,
    ...
    // Clear path cell (matching original: currentLine is x, currentCol is y)
    clearPathCell(startLine, startCol, endLine, endCol) {
        if (!this.ctx) return;
        
        const startX = startLine * this.cellSize + BOX_SPACE + SDPAD + this.boxSize / 2;
        const startY = startCol * this.cellSize + BOX_SPACE + SDPAD + this.boxSize / 2;
        const endX = endLine * this.cellSize + BOX_SPACE + SDPAD + this.boxSize / 2;
        const endY = endCol * this.cellSize + BOX_SPACE + SDPAD + this.boxSize / 2;
