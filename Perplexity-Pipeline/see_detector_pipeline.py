...
# ==== KONFIGURASI PATH ====
# Ganti nama file di sini kalau di mesin kamu beda
...
def preprocess_dynamic(all_dyn: pd.DataFrame) -> pd.DataFrame:
    """Bersihkan dan tambahkan kolom helper di log dinamis per-request."""
    d = all_dyn.copy()
...
def aggregate_dynamic(all_dyn: pd.DataFrame) -> pd.DataFrame:
    """Agregasi fitur perilaku per extension_id."""
...
# ==========================
# 3. BUILD DATASET (STATIC + DYNAMIC)
# ==========================
...
# ==========================
# 4. TRAINING + CV + TEST HOLDOUT
# ==========================

def train_and_evaluate_full(X, y, groups, feature_cols, test_size: float = 0.2):
    """Pipeline lengkap: train/test split per extension, CV di train, evaluasi di test, feature importance."""
    ...
    # --- 4.1 Train/Test split per extension ---
    ...
    print('Train size:', len(y_train), 'extensions; Test size:', len(y_test), 'extensions')

    # --- 4.2 Cross-validation di training set ---
    ...
    for metric in ['train_accuracy', 'train_precision', 'train_recall', 'train_f1', 'test_accuracy', 'test_precision', 'test_recall', 'test_f1']:
        vals = scores[metric]
        print(f"{metric}: {vals.mean():.4f} ± {vals.std():.4f}")

    # --- 4.3 Fit final model di seluruh training dan evaluasi di test ---
    rf = base_rf
    ...
    # --- 4.4 Feature importance (latih di seluruh data untuk insight) ---
    rf_all = base_rf
    rf_all.fit(X, y)
    importances = rf_all.feature_importances_
    fi = sorted(zip(feature_cols, importances), key=lambda x: x[1], reverse=True)

    print('===== Top 20 feature importances (fit on ALL data) =====')
    for name, imp in fi[:20]:
        print(f"{name:30s} {imp:.4f}")

    # --- 4.5 Simpan prediksi test ke CSV untuk dokumentasi ---
...
def main():
    ...
    model, model_all = train_and_evaluate_full(X, y, groups, feature_cols)

    import joblib
    joblib.dump({'model_train': model, 'model_all': model_all, 'feature_cols': feature_cols}, 'see_rf_model_full.joblib')
    print("Model(s) saved to see_rf_model_full.joblib")
