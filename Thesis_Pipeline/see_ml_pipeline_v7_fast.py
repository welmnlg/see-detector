"""
SEE Detector — ML Pipeline v7 (Fast GridSearch + 10-Fold CV)
=================================================================
Strategi:
1. Drop `risk_score`. Hanya gunakan `permissions`.
2. Ekstraksi fitur dinamis mendalam (payload, URL, dll).
3. Evaluasi menggunakan Stratified 10-Fold CV pada seluruh dataset.
4. Gunakan GridSearchCV untuk mempercepat pencarian parameter.
=================================================================
"""
import os
import sys
...
OUTPUT_CSV    = os.path.join(RESULTS_DIR, "dataset_v7_payload.csv")
MODEL_PATH    = os.path.join(MODELS_DIR, "see_rf_model_v7.pkl")
FEATURES_PATH = os.path.join(MODELS_DIR, "feature_names_v7.pkl")
...
def find_best_model_params_fast(X, y):
    print("  [*] Mencari hyperparameter optimal dengan GridSearchCV (5-Fold)...")
    param_grid = {
    ...
    # We use f1_macro to balance both classes in the search
    ...
    print("  [*] Mengevaluasi performa menggunakan 10-Fold CV pada seluruh dataset...")
    cv10 = StratifiedKFold(n_splits=10, shuffle=True, random_state=42)
    y_pred_cv = cross_val_predict(best_rf, X, y, cv=cv10)
    
    return best_rf, y_pred_cv
...
def train_evaluate(df):
    ...
    best_rf, y_pred = find_best_model_params_fast(X_arr, y)
...
def main():
    print("\n" + "=" * 66)
    print("  SEE DETECTOR - FULL ML PIPELINE v7 (Fast CV)")
